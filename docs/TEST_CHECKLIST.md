...
| 3-5 | **비플레이어 물리 자극으로도 함정 발동** (`FLR-028`) | VERIFIED | ✅ `test_nonplayer_stimulus_e2e.gd` — production 어댑터(`TrapSensor`) 경유 |
| 3-8 | 플레이어 진입도 **같은 어댑터**를 쓴다 (경로 단일화) | VERIFIED | ✅ `main`이 `TrapRuntime`/`TrapStimulus`를 직접 부르면 실패 |
| 3-9 | 유배자가 던진 물체가 행동 반경을 넘지 않음 (실제 경로) | VERIFIED | ✅ `CausalSource.THROWN` + 실제 envelope |
...
| 3-4 | 함정 단서를 **플레이 화면에서** 인지 가능 | **PLAYTESTED** | ✅ 오너 2026-08-20 — F1 없이 플레이, "잘 보여". 밀도 조정은 `PHASE 8`로 보류 |

> **PHASE 4 자동 검증 (2026-08-21).** GPT 재검토(`P4-REV-001~005`) 반영 후
> 단위 **1254단언** + 통합 **139단언**, 실패 0.
> `4-2` 오너 PLAYTEST(손맛)만 남았다 — `CBT-008`이 **PLAYTESTED 필수**로 지정한 항목이다.
>
> 수치 이력: 1004/61 (PHASE 3 종료) → **1209/86** (PHASE 4).
>
> ### PHASE 4에서 늘어난 회귀 테스트
>
> | 파일 | 지키는 것 |
> |---|---|
> | `test_combat.gd` | `CBT-004` 크리티컬 비확률 · `CBT-010` 압축 · `CBT-011` 비노출 · `CBT-012` 데이터화 |
> | `test_combat_e2e.gd` | 실제 입력 → 적 처치 → 반격 → 정지 → 대시 → 던지기 → 세이브 |
>
> ---
>
> **PHASE 3 완료 — `PLAYTESTED` (2026-08-20).** GPT 3차 재검토 반영 후
> 단위 **1004단언** + 통합 **61단언**, 실패 0 (`P3-REV-008` 기준).
>
> 수치 이력: 929/25 (P3-T7) → 986/25 (`P3-REV-001~004`) → 998/37 (`P3-REV-005~007`)
> → **1004/61** (`P3-REV-008` + 테스트 유실 가드).
>
> `3-4` 오너 판정: F1(개발 오버레이) 없이 플레이해 **단서가 "잘 보인다"**.
> `FLR-011`의 "발동 전에 알아챌 수 있는 유효 단서" 요건은 greybox 수준에서 충족됐다.
> 실제로 치명 함정 `tr_pit_maze`를 **보고서 일부러** 밟았다 — 못 보고 밟은 것이 아니다.
>
> ⏸ **밀도·진하기 조정은 `PHASE 8`로 보류** (오너 판단).
> greybox 사각형 위에서 튜닝해봐야 도트가 들어오면 다시 해야 한다.
> 인계서도 같은 계획이었다 — "PHASE 3에서는 기능적/greybox 인지 가능성을 먼저 검증".
>
...
| # | 항목 | 필요 등급 |
| --- | --- | --- |
| 4-1 | 세이브/로드 후 전투 상태가 무결하다 | VERIFIED | ✅ `test_combat_e2e.gd` — 죽은 적이 되살아나지 않고 체력 보존 |
| 4-3 | 크리티컬이 **확률 굴림이 아니다** (`CBT-004`) | VERIFIED | ✅ 사건 없이 100회 크리티컬 0건 + 난수 소스 가드 |
| 4-4 | 성장곡선이 **압축**된다 (`CBT-010`) | VERIFIED | ✅ 효율 380배 → 스케일링 √380배 |
| 4-5 | `BodyResilience`가 결과에 노출되지 않는다 (`CBT-011`) | VERIFIED | ✅ |
| 4-6 | 무기별 스탯 비중이 **데이터**에 있다 (`CBT-012`) | VERIFIED | ✅ 코드에 무기/스탯 하드코딩 시 실패 |
| 4-7 | 명중이 굴림이 아니라 **공간**으로 결정된다 (`CBT-008`) | VERIFIED | ✅ 리치·방향·결정성 + 난수 가드 |
| 4-8 | 실제 입력으로 전투가 성립한다 (적 처치 + 반격) | VERIFIED | ✅ `Main.tscn` + 실제 입력 E2E |
| 4-9 | 전술 정지 중 월드가 멈춘다 (`CBT-001`) | VERIFIED | ✅ 정지 중 적 이동 0 |
| 4-10 | 정지가 **완전**하다 — 플레이어 이동·대시·줍기·함정 전부 | VERIFIED | ✅ `test_pause_and_save_e2e.gd` (`P4-REV-001`) |
| 4-11 | 적 위치·공격 진행·모드가 저장/복원된다 | VERIFIED | ✅ 씬 노드가 아니라 데이터가 정본 (`P4-REV-002`) |
| 4-12 | 큰 delta에서도 유효 구간이 유실되지 않는다 | VERIFIED | ✅ coarse/fine 동일 피해 (`P4-REV-003`) |
| 4-13 | 투사체가 대상·벽·함정을 건너뛰지 않는다 | VERIFIED | ✅ 경로 스윕 (`P4-FIX-002` `P4-REV-005`) |
| 4-14 | 임시 DESIGN 수치를 테스트가 **고정하지 않는다** | VERIFIED | ✅ 구조 불변식만 (`P4-REV-004`, 오너 승인) |
| 4-15 | 조준이 마우스·키보드 병행이고 마지막 입력이 이긴다 | VERIFIED | ✅ 순수 함수로 검증 (`P4-FIX-003`) |
| 4-16 | 걷기만으로 조준이 커서 쪽으로 끌려가지 않는다 | VERIFIED | ✅ 화면 좌표로 판정 |
| 4-2 | 공격 선딜·대시의 손맛, 적 공격 회피 난이도 | **PLAYTESTED** | ⏳ 오너 — `CBT-008` 필수 항목 |
