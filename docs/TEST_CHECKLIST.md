...
| # | 항목 | 필요 등급 | 상태 |
| --- | --- | --- | --- |
| 1-1 | 테스트 방에서 벽을 뚫지 않는다 | VERIFIED | ✅ 통합 5케이스 |
| 1-2 | 30 / 60 / 120 FPS 상당의 서로 다른 delta sequence에서도 동일 시간 이동 거리가 같다 | VERIFIED | ✅ 불규칙 delta 포함 |
| 1-3 | 8방향 대각 이동 속도가 직선 이동보다 빠르지 않다 | VERIFIED | ✅ 4방향 전부 |
| 1-4 | 조작감 — 미끄러움·관성이 의도대로인가 | **PLAYTESTED** | ⬜ **오너 확인 필요** |
| 1-5 | 카메라가 플레이어를 따라가고 화면이 흔들리지 않는다 | **PLAYTESTED** | ⬜ **오너 확인 필요** |
...
러너를 둘로 나눈 이유는 `tests/integration_runner.gd` 상단 주석 참고 — 단위는 노드 없이 한 프레임에
끝나지만 충돌 검증은 물리 프레임을 여러 번 돌려야 해서, 한 러너에 넣으면 `D-015` 원칙 1이 깨진다.
