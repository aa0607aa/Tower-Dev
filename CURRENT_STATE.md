...
`docs/reference/FLOOR1_NOVEL_SOURCE_NOTES.md`에 첫 greybox 가설로:
...
를 기록했다. **원문 Canon 숫자가 아니라 PLAYTEST용 DESIGN 시작점**이다.
오너가 범위를 승인/조정하면 Claude가 P2-T2 greybox에 적용한다.

## 열린 Canon 충돌 — PHASE 2 비차단

```text
[CANON CONFLICT]
위치: 오너 제공 소설 초반 캡처 / FLR-014
현재 Canon(A): 1층 스폰 장비는 대체로 저품질이고 초기 대거보다 못할 수도 있다.
소설 원문(B): 1층 스폰 장비는 질이 나빠도 초기 대거보다는 낫다는 취지다.
필요한 결정: A 유지 / B 복원 / 종류별로 다름으로 재정의
영향 범위: PHASE 3 아이템 드롭·초반 무기 밸런스
```

이 충돌은 1층 지형 크기/greybox와 무관하므로 P2-T2를 막지 않는다.
...
1. **[오너]** P2-T2 greybox 시작 범위(위 PROPOSAL)를 승인/조정.
2. **[오너]** FLR-014 무기 품질 충돌은 PHASE 3 전에 결정.
3. **[오너+GPT]** `P2-T0`/`P2-T1` 구현물 독립 검토 예정.
4. **[Claude]** 범위 확정 뒤 P2-T2 greybox + loader 진행.
