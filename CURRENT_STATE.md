...
- **PHASE 4 (반실시간 전투)**: **자동 검증 완료, 오너 PLAYTEST 대기**
  ...
  - [x] `P4-FIX-001` 던진 돌이 함정을 터뜨리지 못하던 문제 (오너 발견)
  - [x] `P4-FIX-002` 투사체 칸 건너뛰기·벽 관통·허용 영역 누출 (오너 발견)
  - [x] `P4-REV-001` **완전 정지가 실제로 완전하지 않던 문제** (GPT)
  - [x] `P4-REV-002` 전투 세이브 무결성 — 씬 노드가 정본이던 문제 (GPT)
  - [x] `P4-REV-003` 큰 delta에서 유효 구간 유실 (GPT)
  - [x] `P4-REV-005` 투사체 대상 터널링 (GPT)
  - [x] `P4-REV-004` 오너 승인 — 임시 DESIGN baseline 허용, **구조 불변식만 테스트**
  - [x] `P4-FIX-003` **조준이 이동 방향으로만 갱신되던 문제** (오너 발견) —
        마우스+키보드 병행으로 변경 (오너 결정 2026-08-21)
...
- 단위 **1254 단언** · 통합 **139 단언**, 실패 0 (1218/91 → 1248/132 → **1254/139**)
