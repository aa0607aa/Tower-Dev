...
- 결정 이력 (`DECISIONS.md`) — `D-001~D-015`, `D-017` Resolved. `D-016`은 PHASE 2 설계 Proposal.
...
- `FLR-023~027` — 실제 월드 일부인 층, 유배자별 맵의 끝, NPC/야생동물 경계 통과, 와이드 맵,
  홀수층 개인 목표/짝수층 공동 목표, 파괴 가능한 물질 지형.
...
- `FLR-017` — `map_bounds`를 세계 끝으로 보지 않고 **유배자별 접근 경계("맵의 끝")**로 정밀화.
- `FLR-014` — 고정 파밍 지점 `tier_hint`를 두지 않는 오너 결정 반영.
- `D-016` PHASE 2 설계안 v2:
  - 최초안의 `trap type 랜덤`, `stair_candidates[] 고정`, `map_bounds = 세계 끝` 전제 폐기.
  - 오너 확정: 함정 고정 / 계단 층 진입 / tier_hint 없음 / 실체화 결과 저장 / 계단 좌표 고정·파괴 불가.
  - 최종 승인 대기: `WorldTerrainDefinition/TerrainBody → FloorDefinition → FloorState` 구조와
    엔진 후보/검증 + 선택적 AI rank + seed top-band 계단 계산 파이프라인.
...
- `CURRENT_STATE.md`와 월간 로그 인계 상태 동기화.
...
- **PHASE 2는 아직 구현 금지.** `D-016`의 마지막 공간/계단 계산 구조를 오너와 확정한 뒤 착수한다.
- 1층 초기 지형은 고정이지만 플레이 중 지형 파괴는 동적 mutation으로 허용된다.
